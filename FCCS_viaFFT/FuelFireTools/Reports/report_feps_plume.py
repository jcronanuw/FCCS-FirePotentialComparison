...
def add_header_content(elements):
    ...
    # Title
    ptext = '<font size={}>FEPS 2.0 Plume Rise and Buoyancy Report</font>'.format(const.FONTSIZE_TITLE)
    ...
    header_data.append(['Fire Size:', '{}'.format(unitsize)])
...
Briggs stability,Buoyancy efficiency,Entrainment efficiency,Combustion efficiency,Plume heat rise ,Briggs buoyancy ,Briggs maximum heat rise (ft),Maximum plume height (ft),Minimum plume height (ft),Lofted PM2.5 ,Drift PM2.5
(ft sec-1)				(Btu)	(ft4 sec-3)					
    '''
    df = pd.read_csv(datafile)
    # truncate output when CO goes below threshold
    idx_last = df.loc[df['buoy_eff'] >= BOUYANCY_EFF_THRESHOLD].last_valid_index() + 1
    df = df.ix[0:idx_last]
    df = df.fillna(0.0)
    ...
    sub_df = pd.DataFrame(
        {
            'Hour': df.index,
            'Plume heat\nrelease (BTU)': df.get('heat'),
            'Min plume\nheight (ft)': df.get('plume_bot').round(2),
            'Max plume\nheight (ft)': df.get('plume_top').round(2),
            'Buoyancy\nefficiency (0-1)': df.get('buoy_eff').round(2),
            'Entrainment\nefficiency (0-1)': df.get('ent_eff').round(2),
            'Combustion\nefficiency (0-1)': df.get('cons_eff').round(2),
            'Briggs\nbuoyancy (m4 sec-3)': df.get('briggs_buoy').round(2),
            'Briggs stability\n(ft sec-1)': df.get('briggs_stab').round(5),
        }
    )
...
def assemble_document(datafile, outfile):
    ...
    cols = [ # total of 825 is about page maximum
        40,
        75,
        65,
        75,
